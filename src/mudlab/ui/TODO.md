...
- [x] Imported/added specimen goniometer (2026-08-05) - Specimen.__init__ left goniometer=None, so import + Add specimen produced a specimen with no goniometer and Edit Specimen greyed the whole Goniometer tab. Now defaults Goniometer() (CuKα; a .mud load overwrites it); tab is editable. verify_specimen_goniometer.py. FOLLOW-UP: populate the goniometer from .xrdml/.raw instrument metadata on import (currently defaults only).
